...
import { NavyLine, PatientDetailBody } from '@/components/loading/PatientShellShape';
...
//
// ─── Streamed in two tiers ─────────────────────────────────────────────
//
// The FIRST query below (`rawPlan` in the page function) is deliberately
// the plan row WITHOUT its `payments` embed — that embed is the expensive
// part (a join across every instalment) and every branching decision this
// page makes (redirect to /confirm, render DeclinedPlanDetail instead, or
// show the normal screen) only needs the plan's own scalar columns. That
// lean row is also everything the header's practice name needs, so the
// header (and the "Left to pay" label under it) can render immediately —
// only the NUMBER, the ladder and the schedule below Suspend on the
// heavier query (loadPlanDetail), which re-fetches the row WITH payments
// plus the cards. Both Suspended pieces below share that one fetch via
// cache(), so it still only runs once per request.
...
function practiceNameFrom(practicesRaw: { name: string } | { name: string }[] | null): string {
  if (!practicesRaw) return 'Unknown Practice';
  return Array.isArray(practicesRaw) ? (practicesRaw[0]?.name ?? 'Unknown Practice') : practicesRaw.name;
}

const loadPlanDetail = cache(async (planId: string, userId: string) => {
  ...
  // Shouldn't happen — the page's own fast probe already confirmed this
  // row exists and belongs to this patient before Suspending on this.
  ...
  return { rawPlan, payments, prog, total, chargeCard, nextDueNumber, today, outstanding, outstandingTotalCents, nextOut, isActive };
});

async function PlanHero({ planId, userId }: { planId: string; userId: string }) {
  const detail = await loadPlanDetail(planId, userId);
  if (!detail) return null;
  const { rawPlan, prog, total } = detail;
...
async function PlanBody({ planId, userId, refSegments }: { planId: string; userId: string; refSegments: string[] }) {
  const detail = await loadPlanDetail(planId, userId);
  if (!detail) return null;
  const { rawPlan, payments, prog, chargeCard, nextDueNumber, today, outstanding, outstandingTotalCents, nextOut, isActive } = detail;
...
export default async function PlanDetailPage({ params }: { params: Promise<{ planId: string }> }) {
  ...
  const practiceName = practiceNameFrom(rawPlan.practice as { name: string } | { name: string }[] | null);
    ...
    <>
      ...
      <div className="mt-[24px]">
        ...
        <Suspense
          fallback={
            <>
              <NavyLine w="w-40" h="h-10" className="mt-[11px]" />
              <NavyLine w="w-full" h="h-2" className="mt-[18px] rounded-full" />
              <NavyLine w="w-36" h="h-3" className="mt-[10px]" />
            </>
          }
        >
          <PlanHero planId={planId} userId={user.id} />
        </Suspense>
      </div>
    </>
  );
  ...
  return (
    <PatientScreen header={header} sheetClassName="px-[18px] pt-5 pb-6">
      <Suspense fallback={<PatientDetailBody label="Loading your plan" cards={2} />}>
        <PlanBody planId={planId} userId={user.id} refSegments={refSegments} />
      </Suspense>
    </PatientScreen>
  );
}
