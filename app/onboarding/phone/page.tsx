...
export default async function PhoneStep() {
  ...
  const { data: profile } = await supabase
    .from('profiles')
    .select('role, phone, phone_verified_at, sa_id_number, salary_day, salary_amount, credit_check_status, liveness_verified_at, onboarding_completed, terms_accepted_at')
    .eq('id', user.id)
    .maybeSingle();
