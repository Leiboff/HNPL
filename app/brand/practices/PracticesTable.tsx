...
function Yes({ children }: { children: React.ReactNode }) {
  return <span style={{ color: brand.ink }}>{children}</span>;
}
...
export default function PracticesTable({ practices }: { practices: BrandPracticeSetup[] }) {
  ...
  return (
    <section aria-labelledby="brand-practices-heading" className="space-y-4">
      <div className="flex items-baseline justify-between gap-3">
        <h2 id="brand-practices-heading" className="text-base font-semibold" style={{ color: NAVY }}>
          Your practices
        </h2>
        <p className="text-xs" style={{ color: brand.muted }} data-testid="brand-practices-summary">
          {needing === 0
      ...
      {/* The table stays a REAL <table> — role="row" is asserted on directly
          (see PracticesTable.test.tsx), so the calmer look below is achieved
          with padding, colour and a decorative progress bar rather than by
          restructuring the rows into cards. */}
      <div className="rounded-3xl bg-white overflow-x-auto" style={{ boxShadow: cardShadow }}>
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left" style={{ background: brand.mint }}>
              <th className="px-5 py-3 font-semibold rounded-tl-3xl" style={{ color: brand.tealDeep }}>Practice</th>
              <th className="px-4 py-3 font-semibold" style={{ color: brand.tealDeep }}>Approval</th>
              <th className="px-4 py-3 font-semibold" style={{ color: brand.tealDeep }}>Bank account</th>
              <th className="px-4 py-3 font-semibold" style={{ color: brand.tealDeep }}>Practitioners</th>
              <th className="px-4 py-3 font-semibold rounded-tr-3xl" style={{ color: brand.tealDeep }}>Front desk till</th>
            </tr>
          </thead>
            ...
            {practices.map((p) => {
              ...
              return (
                <tr
                  ...
                  className={p.needsAttention ? 'bg-amber-50/30' : 'hover:bg-gray-50/70 transition-colors'}
                >
                  <td className="px-5 py-4 align-top">
                    <div
                      className={p.needsAttention ? 'border-l-2 border-amber-500 pl-3 -ml-3' : undefined}
                    >
                      <Link
                      ...
                      <p className="text-[11px] mt-0.5" style={{ color: brand.faint }}>
                        {[p.suburb, p.city].filter(Boolean).join(', ') || '—'}
                      </p>

                      {/* Decorative — the same "N of total set up" progress the
                          practice's own checklist card shows, so this list reads
                          at a glance without opening a row. Not a testid: the
                          exact counts are already covered via `missing` below. */}
                      <div className="flex items-center gap-2 mt-2 max-w-32">
                        <div className="flex-1 h-1.5 rounded-full overflow-hidden" style={{ background: brand.line }}>
                          <div
                            className="h-full rounded-full"
                            style={{
                              width: `${p.total > 0 ? (p.doneCount / p.total) * 100 : 100}%`,
                              background: p.doneCount === p.total ? brand.teal : brand.amber,
                            }}
                          />
                        </div>
                        <span className="text-[10px] whitespace-nowrap" style={{ color: brand.faint }}>
                          {p.doneCount}/{p.total}
                        </span>
                      </div>
                      ...
                      {p.needsAttention && (
                        <span
                          ...
                          className="inline-block mt-2 rounded-full bg-amber-100 px-2 py-0.5 text-[11px] font-semibold text-amber-800"
                        >
                          Action needed
                        </span>
                      )}
                  ...
                  <td className="px-4 py-4 align-top">
                    <span
                      ...
                      className={`inline-block rounded-full px-2.5 py-1 text-[11px] font-medium ${statusClass(p.status)}`}
                    >
                      {p.status}
                    </span>
                  </td>

                  <td className="px-4 py-4 align-top text-xs" data-testid={`brand-practice-banking-${p.practiceId}`}>
                    {p.done.banking ? <Yes>Set</Yes> : <No>Not set</No>}
                  </td>

                  <td className="px-4 py-4 align-top text-xs" data-testid={`brand-practice-providers-${p.practiceId}`}>
      ...
      <p className="text-[11px]" style={{ color: brand.muted }} data-testid="brand-practices-approval-note">
