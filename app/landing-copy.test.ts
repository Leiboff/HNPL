...
describe('Approved slogans — each exactly once, in the right section', () => {
  ...
  it('S3 "Pay your bill in smaller doses." is the calculator h2', () => {
    expect(CALC()).toMatch(/<h2>Pay your bill <span>in smaller doses\.<\/span><\/h2>/);
    expect(count('smaller doses')).toBe(1);
  });

  it('"A bill of health you can afford." is the How-it-works h2', () => {
    expect(HOW()).toMatch(/<h2>A bill of health <span>you can afford\.<\/span><\/h2>/);
