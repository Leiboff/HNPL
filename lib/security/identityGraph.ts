...
/**
 * Where the linked identities' plans were billed.
 *
 * THE COLLUSION CASE. The practice is paid 94% UPFRONT, which makes the
 * payout the exfiltration channel and a captured or colluding practice the
 * highest-value attack on this business model. A ring of rented identities
 * has to bill through somebody; if the same somebody is behind all of them,
 * that is the shape of the attack.
 *
 * Absent (undefined) when the question could not be answered — which is
 * deliberately different from "answered, and they are spread widely".
 */
export type PracticeConcentration = {
  /** Distinct verified identities sharing any of the applicant's keys. */
  linkedIdentities:  number;
  /** Plans belonging to those identities, in any status. */
  linkedPlans:       number;
  /** How many practices billed those plans. */
  distinctPractices: number;
};

export type RingObservation = {
  links: IdentityLink[];
  practiceConcentration?: PracticeConcentration;
};

/**
 * Below this much linkage, concentration means nothing and is not scored.
 *
 * With two linked identities and one practice, "100% concentrated" is the
 * overwhelmingly likely innocent outcome — two people who went to the same
 * clinic. The figure only starts carrying information once there are
 * enough independent people for spread to have been the expectation.
 *
 * This is the whole reason the RPC returns three numbers instead of the
 * boolean an earlier draft used: a caller-computed "all at one practice"
 * flag is trivially true for small groups, so it would have fired hardest
 * on the smallest and most innocent ones.
 */
const CONCENTRATION_MIN_IDENTITIES = 4;
const CONCENTRATION_MIN_PLANS      = 4;
...
export function assessRing(observation: RingObservation): RingAssessment {
  ...
  const concentration = scorePracticeConcentration(observation.practiceConcentration);
...
/**
 * Score how tightly the linked identities' billing concentrates.
 *
 * Graded rather than binary, because "eight identities, one practice" and
 * "eight identities, six practices" are different findings and a boolean
 * throws that away.
 *
 * Capped well below any threshold that matters on its own, and it is not a
 * correlation KIND — so it never registers in contributingKinds and can
 * never satisfy the corroboration rule. A rural town with one clinic
 * cannot be turned into a ring by this signal alone, however concentrated
 * its patients look.
 */
function scorePracticeConcentration(c: PracticeConcentration | undefined): RingSignal | null {
  if (!c) return null;
  if (c.linkedIdentities < CONCENTRATION_MIN_IDENTITIES) return null;
  if (c.linkedPlans      < CONCENTRATION_MIN_PLANS)      return null;
  if (c.distinctPractices < 1) return null;

  // Plans per practice. At 1.0 the billing is as spread as it could be;
  // it climbs as the same practice appears behind more of the group.
  const perPractice = c.linkedPlans / c.distinctPractices;

  // Everything through one practice is the strong case; a heavy but not
  // total skew still says something.
  if (c.distinctPractices === 1) {
    return {
      code: 'practice_concentration',
      weight: 24,
      detail: `${c.linkedPlans} plans across ${c.linkedIdentities} linked identities, all billed by one practice`,
    };
  }
  if (perPractice >= 3) {
    return {
      ...
      weight: 12,
      detail:
        `${c.linkedPlans} plans across ${c.linkedIdentities} linked identities, ` +
        `concentrated in ${c.distinctPractices} practices`,
    };
  }
  return null;
}
