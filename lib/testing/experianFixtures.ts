 ...
 *
 * Adapted from docs/experian/fixtures.ts — the verified reference, which stays
 * unmodified. Byte-identical to it apart from this note and two additions at
 * the foot of FIXTURES (`sts_fallback_after_thin_su` and `sts_band4`), which
 * exercise the pVersion 4.0 Sigma Transcend fallback. Both are SYNTHETIC and
 * flagged as such: our branch had the fallback switched off when the real
 * captures were taken, so no genuine STS payload exists to capture yet.
 *
 * Lives in lib/testing/ rather than beside lib/experian/, matching
 * rateLimitTestMock.ts and riskTestMock.ts — module-specific test doubles are
 * collected here in this repo.
 *
...
export const FIXTURES = {
  ...
  // ---- SYNTHETIC: the pVersion 4.0 Sigma Transcend fallback -------------------
  //
  // SYNTHETIC, and it has to be: when 4.0 was tested the fallback was not yet
  // activated on our branch, so a genuine thin file returned SU -1 with MI62 and
  // NO STS card (that payload is real_su_thin_file above). Experian have since
  // confirmed the fallback is real and that our branch was the limitation.
  //
  // This is the shape the response takes once it is switched on: the primary SU
  // card unusable, with a scored STS card beside it. The point of the fixture is
  // that the preference order falls SU → SS → STS on its own, and that STS is
  // banded on the STS table — whose bands are far tighter than SU's — rather than
  // on the primary card's.
  sts_fallback_after_thin_su:
    '{"idNumber":"REDACTED","results":[' +
    '{"resultType":"SU","score":"-1","reasons":[{"reasonCode":"MI62","reasonDescription":"MI62-Customer has no Accounts that have been open for more than 3 months"}]},' +
    '{"resultType":"STS","score":"610","reasons":[{"reasonCode":"TS14","reasonDescription":"TS14-Limited credit history depth"}]}]}',

  /** STS 610 is band 4 on the STS table (bounds 597/602/608/621) and band 1 on SU's. */
  sts_band4: '{"results":[{"resultType":"STS","score":"610","reasons":[]}]}',
} as const;
