 ...
 *
 * Adapted from docs/experian/client.ts, which is the verified reference and
 * stays unmodified. Everything load-bearing below was validated against the
 * LIVE service; the deviations from the reference are the pVersion note and
 * the move of configuration into ./config.ts.
 *
 * Spec: "Experian Person Get Score Integration Specification" v2.1, corrected
 * against the live WSDL at https://apis-uat.experian.co.za/GetPersonScore?wsdl
 * — which disagrees with the PDF in three places, all of them load-bearing
 * (see notes on buildRequest below).
 *
 * SOAP over 443, not REST over 9443. Measured reason: 9443 is source-
 * restricted. It is reachable from a South African connection and times out
 * from cloud datacentre IPs, which is what Vercel functions run on. 443
 * answered from both. Do not "fix" this back to REST.
 *
 * SERVER ONLY. The request body contains the password in cleartext. Never log
 * it, never attach it to an error report, never return it from a route
 * handler. There is deliberately no console.* call anywhere in this file —
 * lib/experian/experian.adversarial.test.ts asserts that on the source text.
 */
...
export interface ExperianConfig {
  ...
  /**
   * pVersion, max 5. Read from the environment (see ./config.ts) so it can be
   * changed without a deploy, defaulting to '4.0'.
   *
   * What each version does on OUR branch, confirmed against production rather
   * than taken from the spec:
   *
   *   1.0 — returns NLR and CPA.
   *   2.0 — returns exactly one card, the branch's activated score, currently
   *         SU. The spec says this yields Compuscore V3 (CT/CU); it does not.
   *   4.0 — as 2.0, and additionally falls back to Sigma Transcend (STS) when
   *         the primary card is unavailable.
   *
   * When 4.0 was tested the fallback was not yet activated on this branch, so
   * 2.0, 3.0 and 4.0 returned byte-identical single-card responses and a
   * genuine thin file came back as SU score -1, reason MI62, with no STS card.
   * Experian have since confirmed the fallback is real and that our branch was
   * the limitation — which is why SCORECARD_PREFERENCE carries STS and picks
   * it up automatically once the branch is switched on.
   *
   * A wrong value returns -105, and that is still billable.
   */
  pVersion: string;
  timeoutMs: number;
}
 ...
 *
 * Both properties are pinned by lib/experian/client.test.ts, including adversarially
 * (that a default-namespace envelope is never produced), because a "tidy-up" that
 * changes either one costs a billable transaction to rediscover.
 */
